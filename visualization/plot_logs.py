...
def main(args):
    ...
    epochs = find_reg(r"Epoch: (.*)  Speed", rows)
    max_epoch = int(max(epochs))

    acc_lfw = find_reg(r"\[lfw\]\[[0-9]*\]Accuracy-Flip: (.*)\+-", rows) * 100
    max_acc_lfw = round(np.max(acc_lfw), 2) if len(acc_lfw) > 0 else 0
    acc_agedb = find_reg(r"\[agedb_30\]\[[0-9]*\]Accuracy-Flip: (.*)\+-", rows) * 100
    max_acc_agedb = round(np.max(acc_agedb), 2) if len(acc_agedb) > 0 else 0
    acc_cfpfp = find_reg(r"\[cfp_fp\]\[[0-9]*\]Accuracy-Flip: (.*)\+-", rows) * 100
    max_acc_cfpfp = round(np.max(acc_cfpfp), 2) if len(acc_cfpfp) > 0 else 0
    acc_calfw = find_reg(r"\[calfw\]\[[0-9]*\]Accuracy-Flip: (.*)\+-", rows) * 100
    max_acc_caflw = round(np.max(acc_calfw), 2) if len(acc_calfw) > 0 else 0
    acc_cplfw = find_reg(r"\[cplfw\]\[[0-9]*\]Accuracy-Flip: (.*)\+-", rows) * 100
    max_acc_cplfw = round(np.max(acc_cplfw), 2) if len(acc_cplfw) > 0 else 0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--log_path",
        type=str,
        default="../output/ExFace_SG3_CosFace_RA",
