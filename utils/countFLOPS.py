...
def count_parameters(model, m_name):
    """ counts model parameters
    args:
        model: model
        m_name: model name for return string
    return:
        string with total and trainable parameters
    """
    total_params = sum(p.numel() for p in model.parameters())
    train_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    return f"{m_name}: Total parameters: {total_params}      Trainable parameters: {train_params}"
